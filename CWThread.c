...
#ifdef CW_USE_NAMED_SEMAPHORES
	if (sem_getvalue(semPtr->semPtr, valuePtr) < 0)
	{ // note: broken on Mac OS X? Btw we don't need it
		CWErrorRaiseSystemError(CW_ERROR_GENERAL);
	}
#else
...
CWBool CWThreadCreateTimedSem(CWThreadTimedSem *semPtr, int value)
{
	...
	if ((((*semPtr)[0] = socket(AF_LOCAL, SOCK_DGRAM, 0)) < 0) ||
		(((*semPtr)[1] = socket(AF_LOCAL, SOCK_DGRAM, 0)) < 0))
	{ // create a pair of datagram unix domain socket
		close((*semPtr)[0]);
