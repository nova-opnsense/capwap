...
CWBool nl80211CmdGetPhyInfo(int indexPhy, struct WTPSinglePhyInfo *singlePhyInfo)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Get phy info error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdSetNewInterface(int indexPhy, WTPInterfaceInfo *interfaceInfo)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set new interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
	...
	log_info("[NL80211 INFO] Interface %s created", interfaceInfo->ifName);
	return CW_TRUE;
...
CWBool nl80211CmdSetNewMonitorInterface(int indexPhy, WTPInterfaceInfo *interfaceInfo)
{
	...
	*/
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Create monitor interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdDelInterface(int indexPhy, char *ifName)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Del interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdSetInterfaceAPType(char *interface)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set AP interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
	msg = NULL;
...
CWBool nl80211CmdSetInterfaceSTAType(char *interface)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set STA interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
	msg = NULL;
...
CWBool nl80211CmdSetChannelInterface(char *interface, int channel)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set channel interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211_get_channel_width(char *interface)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Get Channel width error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdGetChannelInterface(char *interface, int *channel)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Get channel interface error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdStartAP(WTPInterfaceInfo *interfaceInfo)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Start AP beacon error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdNewStation(WTPBSSInfo *infoBSS, WTPSTAInfo staInfo)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Add new STA error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdSetStation(WTPBSSInfo *infoBSS, WTPSTAInfo staInfo)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set update STA error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdDelStation(WTPBSSInfo *infoBSS, unsigned char *macAddress)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Del STA error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
*/
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Set BSS error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
	msg = NULL;
...
CWBool nl80211CmdStopAP(char *ifName)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] STOP AP error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
...
int nl80211GetTxqParams(struct nl80211SocketUnit *nlSockUnit, WTPQosValues *qosValues, int ifindex)
{
	...
	{
		log_error("[NL80211 ERROR]: nl80211_send_recv_cb_input");
		return 0;
	}
...
out_err:
	...
	log_error("[NL80211 ERROR] Set AP Type Frame error");
	return -1;
}
						   ...
						   u16 type, const u8 *match, size_t match_len)
{
	...
	if (ret != 0)
	{
		log_error("[NL80211 ERROR] Register frame command failed (type=%u): ret=%d (%s)", type, ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool CW80211SendFrame(WTPBSSInfo *WTPBSSInfoPtr, unsigned int freq, unsigned int wait, char *buf, size_t buf_len, u64 *cookie_out, int no_cck, int no_ack)
{
	...
	if (ret)
	{
		log_error("[NL80211 ERROR]: Frame command failed: ret=%d (%s) (freq=%u wait=%u) nl_geterror: %s", ret, strerror(-ret), freq, wait, nl_geterror(ret));
		goto nla_put_failure;
	}
