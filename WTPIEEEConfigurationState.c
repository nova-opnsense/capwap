											  ...
											  ACInterfaceRequestInfo *interfaceInfo)
{
	...
	while ((completeMsg.offset - offsetTillMessages) < controlVal.msgElemsLen)
	{
		...
		case CW_MSG_ELEMENT_IEEE80211_ADD_WLAN_CW_TYPE:
			if (!(CWParseACAddWlan(&completeMsg, len, interfaceInfo)))
				return CW_FALSE;
			break;
		case CW_MSG_ELEMENT_IEEE80211_DELETE_WLAN_CW_TYPE:
			if (!(CWParseACDelWlan(&completeMsg, len, interfaceInfo)))
				return CW_FALSE;
			break;
		case CW_MSG_ELEMENT_IEEE80211_UPDATE_WLAN_CW_TYPE:
			if (!(CWParseACUpdateWlan(&completeMsg, len, interfaceInfo)))
				return CW_FALSE;
			break;
		default:
